...
NOTE: Kubernetes Pod as Jenkins Build Agent. By defauly these pods are launched with default service account from its namespace. The default service account has no ClusterRole 
to deploy application into k8. So we have to follow these steps to give required permissions to these Build Agent Pods to deploy application into k8
